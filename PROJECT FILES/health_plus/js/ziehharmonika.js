...
(function($) {
	var settings;
	$.fn.harman9 = function(actionOrSettings) {
		if (typeof actionOrSettings === 'object' || actionOrSettings === undefined) {
			// Default settings:
			settings = $.extend({
				// To use a headline tag other than h3, adjust or overwrite harman9.css as well
				headline: 'h3',
		...
		if (actionOrSettings == "open") {
			...
			$(settings.headline).harman9('close');
		} else if (actionOrSettings == "forceCloseAll") {
			// forceCloseAll ignores collapsible setting
			$(settings.headline).harman9('forceClose');
		}
